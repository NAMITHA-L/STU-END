...
type Task = { id: string; title: string; subject: string; priority: Priority; completed: boolean; skipped?: boolean; dueDate: string; dueTime: string }
...
export default function TaskList() {
  ...
  const skipTask = async (task: Task) => {
    const response = await fetch(`/api/tasks/${task.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ skipped: !task.skipped }) })
    if (!response.ok) return toast.error("Could not update task status")
    setTasks((current) => current.map((item) => item.id === task.id ? { ...item, skipped: !task.skipped } : item))
    toast.success(task.skipped ? "Task reopened" : "Task marked skipped")
  }
